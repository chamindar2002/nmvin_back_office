...
abstract class BaseCustomerdetails extends BaseObject  implements Persistent {
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 34; // 34 = CustomerdetailsPeer::NUM_COLUMNS - CustomerdetailsPeer::NUM_LAZY_LOAD_COLUMNS).
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Customerdetails has previously been saved, it will retrieve
	 * related Refundss from storage. If this Customerdetails is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collRefundss === null) {
			if ($this->isNew()) {
			   $this->collRefundss = array();
				...
				$criteria->add(RefundsPeer::CUSTOMERCODE, $this->customercode);
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Customerdetails has previously been saved, it will retrieve
	 * related Tranferedblokss from storage. If this Customerdetails is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collTranferedblokss === null) {
			if ($this->isNew()) {
			   $this->collTranferedblokss = array();
				...
				$criteria->add(TranferedbloksPeer::CUSTOMERCODE, $this->customercode);
	...
	public function getTranferedblokssJoinSales($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		if ($this->collTranferedblokss === null) {
			if ($this->isNew()) {
				...
				$this->collTranferedblokss = TranferedbloksPeer::doSelectJoinSales($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
