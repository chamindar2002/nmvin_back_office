...
abstract class BaseCustomerreceipts extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array CustomerReceiptmethodofpayment[] Collection to store aggregation of CustomerReceiptmethodofpayment objects.
	 */
	protected $collCustomerReceiptmethodofpayments;

	/**
	 * @var        Criteria The criteria used to select the current contents of collCustomerReceiptmethodofpayments.
	 */
	private $lastCustomerReceiptmethodofpaymentCriteria = null;
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->addedby = ($row[$startcol + 12] !== null) ? (int) $row[$startcol + 12] : null;
			$this->addeddate = ($row[$startcol + 13] !== null) ? (string) $row[$startcol + 13] : null;
			$this->addedtime = ($row[$startcol + 14] !== null) ? (string) $row[$startcol + 14] : null;
			$this->lastmodifiedby = ($row[$startcol + 15] !== null) ? (int) $row[$startcol + 15] : null;
			$this->lastmodifieddate = ($row[$startcol + 16] !== null) ? (string) $row[$startcol + 16] : null;
			$this->lastmodifiedtime = ($row[$startcol + 17] !== null) ? (string) $row[$startcol + 17] : null;
			$this->deletedby = ($row[$startcol + 18] !== null) ? (int) $row[$startcol + 18] : null;
			$this->deleteddate = ($row[$startcol + 19] !== null) ? (string) $row[$startcol + 19] : null;
			$this->deletedtime = ($row[$startcol + 20] !== null) ? (string) $row[$startcol + 20] : null;
			$this->resetModified();
			...
			return $startcol + 21; // 21 = CustomerreceiptsPeer::NUM_COLUMNS - CustomerreceiptsPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->lastCustomerReceiptmethodofpaymentCriteria = null;
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setOldreceiptno($this->oldreceiptno);

		$copyObj->setBlocknumber($this->blocknumber);

		$copyObj->setPaidfor($this->paidfor);
	...
	/**
	 * Clears out the collCustomerReceiptmethodofpayments collection (array).
	 *
	...
	/**
	 * Initializes the collCustomerReceiptmethodofpayments collection (array).
	 *
	 ...
	 * @return     void
	 */
	public function initCustomerReceiptmethodofpayments()
	{
		$this->collCustomerReceiptmethodofpayments = array();
	}
	 ...
	 * Otherwise if this Customerreceipts has previously been saved, it will retrieve
	 * related CustomerReceiptmethodofpayments from storage. If this Customerreceipts is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array CustomerReceiptmethodofpayment[]
	 * @throws     PropelException
	 */
		...
		if ($this->collCustomerReceiptmethodofpayments === null) {
			if ($this->isNew()) {
			   $this->collCustomerReceiptmethodofpayments = array();
				...
				$criteria->add(CustomerReceiptmethodofpaymentPeer::RECEIPTNO, $this->receiptno);

				CustomerReceiptmethodofpaymentPeer::addSelectColumns($criteria);
				$this->collCustomerReceiptmethodofpayments = CustomerReceiptmethodofpaymentPeer::doSelect($criteria, $con);
			}
		} else {
			...
			if (!$this->isNew()) {
				...
				CustomerReceiptmethodofpaymentPeer::addSelectColumns($criteria);
				if (!isset($this->lastCustomerReceiptmethodofpaymentCriteria) || !$this->lastCustomerReceiptmethodofpaymentCriteria->equals($criteria)) {
					$this->collCustomerReceiptmethodofpayments = CustomerReceiptmethodofpaymentPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastCustomerReceiptmethodofpaymentCriteria = $criteria;
	...
	public function countCustomerReceiptmethodofpayments(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		...
		if ($this->collCustomerReceiptmethodofpayments === null) {
			if ($this->isNew()) {
				...
				$count = CustomerReceiptmethodofpaymentPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastCustomerReceiptmethodofpaymentCriteria) || !$this->lastCustomerReceiptmethodofpaymentCriteria->equals($criteria)) {
					...
					$count = count($this->collCustomerReceiptmethodofpayments);
				}
			} else {
				$count = count($this->collCustomerReceiptmethodofpayments);
			}
	...
	public function addCustomerReceiptmethodofpayment(CustomerReceiptmethodofpayment $l)
	{
		...
		if (!in_array($l, $this->collCustomerReceiptmethodofpayments, true)) { // only add it if the **same** object is not already associated
			array_push($this->collCustomerReceiptmethodofpayments, $l);
			$l->setCustomerreceipts($this);
		}
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			if ($this->collCustomerReceiptmethodofpayments) {
				foreach ((array) $this->collCustomerReceiptmethodofpayments as $o) {
