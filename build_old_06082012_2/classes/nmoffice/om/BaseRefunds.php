...
abstract class BaseRefunds extends BaseObject  implements Persistent {
	...
	public function setRefundate($v)
	{
		...
		if ( $this->refundate !== null || $dt !== null ) {
			...
			$currNorm = ($this->refundate !== null && $tmpDt = new DateTime($this->refundate)) ? $tmpDt->format('Y-m-d') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->refundate = ($dt ? $dt->format('Y-m-d') : null);
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 19; // 19 = RefundsPeer::NUM_COLUMNS - RefundsPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = RefundsPeer::doInsert($this, $con);
					...
					$affectedRows += RefundsPeer::doUpdate($this, $con);
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setRefundamount($this->refundamount);

		$copyObj->setRefundate($this->refundate);
