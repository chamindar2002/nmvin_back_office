<?php 
 ...
 class ProjectSummaryDetailsReportAction implements Action{ 
 	 public function execute(){ 
  		//showActionFileOutput(20);
  		if(isset($_POST['rptcode4'])){
 	 		$projectcode = $_POST['projects2'];
 	 		...
 	 		if(sizeof($sales)>0){
				...
				$pdf->SetPageHeadning("Project Summary Break Down Report Of $projectname");
				...
				//$pdf->Cell(190,4,"Project Summary Break Down Report",0,0,'C');
 	 				...
 	 				foreach($sales As $s){
  						...
  						if(sizeof($lastreceipt) == 1){
	  						...
	  						$paid_for = substr(displayPaidfor($receipt_paidfor),0,14);
 	 				...
 	 				$pdf->Output("projectsummarydetails_report.pdf",'D');	
 	 			}
