...
function printProjects_($selected){
	$projectlist = TransferManager::getInstance()->listProjects();
...
function getBlockDetails($blockrefno,$controlname){
	...
	$blockdetails = TransferManager::getInstance()->getBlockDetailsByPk($blockrefno);
	if(sizeof($blockdetails) == 1){
		...
		$customer = TransferManager::getInstance()->getCustomerByPk($customercode);
		if(isset($customer)){
                        $customerName = "<strong>" ."Current Owner". "</strong>" ."<br>";
		...
		$sale = TransferManager::getInstance()->getSaleRefNumberByBlockRefNumber($blockrefno);
		if(sizeof($sale) == 1){
...
function searchCustomer($passportno){
    include_once("TransferManager.php");
    $customer = TransferManager::getInstance()->getCustomerByNicNo($passportno);
    $out = '';
    $customerName = '';
    if(isset($customer)){
    ...
    $out .= $customerName;
    $out .= "<input type='hidden' name=\"ccode_tranferto\" id=\"ccode_tranferto\" value=\"$customercode\">";
    echo $out;
}
