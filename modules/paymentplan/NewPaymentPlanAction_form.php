...
function cancelForm(){
	window.location = "index.php?m=paymentplan&a=NewPaymentPlan";
}
...
function validateForm(){
	...
	var downp =  document.getElementById('downpamnt');
        ...
        var totalpayable = (Number(instalmnt.value)*Number(nofinstall.value) + Number(downp.value))
	...
	if(projects.value == 0){
		...
		projects.focus();	
			...
			nofinstall.focus();
				...
				instalmnt.focus();
			}else if(downp.value == ""  || !IsNumeric(downp.value)){
				alert("Error: Invalid Downpayment.");
				downp.focus();	
				}else if(desc.value.length < 5){
					alert("Error: Invalid Description.");
					desc.focus();
					}else if(amtpayable.value == 0 || amtpayable.value == "" || !IsNumeric(amtpayable.value) || Number(amtpayable.value)<(totalpayable)){
						alert("Error: Amount Payable is invalid."+amtpayable.value + totalpayable);
						amtpayable.focus();
						}else{
							document.frmnewpplan.submit();
							}
}
...
<tr><td>
	<table class='contenttable'>
	  ...
	  <tr>
	  	<td>
	  		<?php include('newpaymentplanForm.php');?>
	  	</td>
	  </tr>
