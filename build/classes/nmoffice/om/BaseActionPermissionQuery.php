...
abstract class BaseActionPermissionQuery extends ModelCriteria
{
     ...
     * @param   ActionPermissionQuery|Criteria $criteria Optional Criteria to build the query from
     *
    ...
    public static function create($modelAlias = null, $criteria = null)
    {
        ...
        $query = new ActionPermissionQuery(null, null, $modelAlias);
     ...
     *
     * @return   ActionPermission|ActionPermission[]|mixed the result, formatted by the current formatter
     */
    public function findPk($key, $con = null)
    {
        ...
        if ((null !== ($obj = ActionPermissionPeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
     ...
     *
     * @return                 ActionPermission A model object, or null if the key is not found
    ...
    {
        $sql = 'SELECT `id`, `module`, `action`, `description`, `systemid` FROM `action_permission` WHERE `id` = :p0';
        try {
        ...
        if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            ActionPermissionPeer::addInstanceToPool($obj, (string) $key);
        }
        $stmt->closeCursor();
     ...
     * @param     PropelPDO $con an optional connection object
     *
     * @return PropelObjectCollection|ActionPermission[]|mixed the list of results, formatted by the current formatter
     */
    ...
    public function filterById($id = null, $comparison = null)
    {
        if (is_array($id)) {
            $useMinMax = false;
            if (isset($id['min'])) {
                $this->addUsingAlias(ActionPermissionPeer::ID, $id['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($id['max'])) {
                $this->addUsingAlias(ActionPermissionPeer::ID, $id['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
     ...
     * Example usage:
     * <code>
     * $query->filterBySystemid(1234); // WHERE systemid = 1234
     * $query->filterBySystemid(array(12, 34)); // WHERE systemid IN (12, 34)
     * $query->filterBySystemid(array('min' => 12)); // WHERE systemid >= 12
     * $query->filterBySystemid(array('max' => 12)); // WHERE systemid <= 12
     * </code>
     *
