...
abstract class BaseRole extends BaseObject  implements Persistent {
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$this->setRid($pk);  //[IMV] update autoincrement primary key
	 ...
	 * related UserRoleRefs from storage. If this Role is new, it will return
	...
	public function getUserRoleRefsJoinUser($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		if ($this->collUserRoleRefs === null) {
			if ($this->isNew()) {
				...
				$this->collUserRoleRefs = UserRoleRefPeer::doSelectJoinUser($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Role has previously been saved, it will retrieve
	 * related RoleActionPermissionRefs from storage. If this Role is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	...
	public function getRoleActionPermissionRefsJoinActionPermission($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		if ($this->collRoleActionPermissionRefs === null) {
			if ($this->isNew()) {
				...
				$this->collRoleActionPermissionRefs = RoleActionPermissionRefPeer::doSelectJoinActionPermission($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
